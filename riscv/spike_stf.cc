...
//    if(stfhandler->stf_trace_register_state)
//    {
...
//    bool start = START_TRACE == insn.bits() && !stfhandler->stf_macro_tracing_active;
//    bool stop = STOP_TRACE == insn.bits() && stfhandler->stf_macro_tracing_active;
//
//    if(start) {        
//        std::cout << "START_TRACE detected at PC: 0x" << std::hex << state->pc << std::dec << std::endl;
////        proc->stf_macro_tracing_active = true;
//        stfhandler->stf_macro_tracing_active = true;
...
//        stfhandler->stf_macro_tracing_active = false;
//        stf_trace_close(proc, state->pc);
//    }
//    
//    return stfhandler->stf_macro_tracing_active;
//}
//
//void stf_trace_open(processor_t* proc, unsigned long PC)
//{
//  stfhandler->stf_trace_file_open = true;
...
//if(LOG_IT)        std::cout << ">>> SPIKE: Traced " << stfhandler->stf_num_traced<< " insts\n" << std::endl;
...
//
//        stfhandler->stf_trace_file_open = false;
//        stfhandler->stf_macro_tracing_active = false;
//        stfhandler->stf_num_traced = 0;
//
//        stf_writer.flush();
//        stf_writer.close();
//
...
//    bool traceable_priv_level = state->prv <= get_highest_priv_mode(stfhandler->stf_priv_modes);
//
//    if(traceable_priv_level)
//    {
////        ++proc->stf_num_traced;
//        ++stfhandler->stf_num_traced;
...
//            skip_record = (state->pc != stfhandler->get_last_pc_stf() + inst_width);
//        }
//
...
//if(LOG_IT)        std::cout << ">>> SPIKE: Last PC " << std::hex << stfhandler->get_last_pc_stf() << std::dec << std::endl;
...
//            if(!stfhandler->stf_disable_memory_records) {
